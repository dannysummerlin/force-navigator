// setup of box
	...
	// Command: Create Fields
		if(ins.substring(0,3) == 'cf ' && ins.split(' ').length < 4) {
			...
			if (visibleWords.length > 0) {
				...
				setVisible("visible")
				input = document.getElementById("sfNavQuickSearch").value
			} else{
				setVisible("hidden")
				posi = -1
			}
	...
	function setVisible(visi){
		var x = document.getElementById("sfNavShadow")
		var t = document.getElementById("sfNavQuickSearch")
		x.style.position = 'relative'
		x.style.visibility = visi
	}
	function setVisibleSearch(visi) {
		var t = document.getElementById("sfNavCommandPrompt")
		t.style.visibility = visi
		if(visi=='visible') document.getElementById("sfNavQuickSearch").focus()
	}
	function lookAt(){
		var ins = document.getElementById("sfNavQuickSearch").value
